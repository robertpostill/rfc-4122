...
Installing rfc-4122 Locally
--------------------------------------

rfc-4122 will be installed so that it is linked directly to the cloned instance of the Git repo on your local machine. It means that any changes you make there will be reflected in all code on your machine that depends on rfc-4122. Likewise, updating rfc-4122 would just be a matter of ``git pull``.

Uninstall any version of rfc-4122 you already have
^^^^^^^^^^^^^^^^^^^^^^^^^^^^^^^^^^^^^^^^^^^^^^^^^^^^^^^^^^^^^

.. code-block:: bash

  raco pkg remove --force rfc-4122
